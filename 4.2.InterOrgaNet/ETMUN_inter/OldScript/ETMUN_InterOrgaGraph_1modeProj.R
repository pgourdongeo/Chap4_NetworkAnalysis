...
## Prep data for classiq ggplot
## Filter WARNING Index are computed without the filter
Fgraph2 <- TdInterOrga %>% activate(nodes) %>% filter(AssoSize < 500) %>% filter(degree>2) # remove Covenant of Mayors (n= 10 000) and Climate Alliance (n =1742) + WWCAM
...
ggplot(Fgraph2Log10, aes(x = AssoSize, y = relative_wdegree_log10))+
  geom_point()

#### Relation between degree and Weigthed degree
#expect non linar one


ggplot(Fgraph2, aes(x = degree, y = wdegree))+
  geom_point()

ggplot(Fgraph2, aes(x = degree, y = wdegree))+
  scale_y_continuous(trans = 'log10')+
  geom_point()+
  labs(y = "Weigthed Degree (log10)")

#### Relation between Size Asso and  degree
ggplot(Fgraph2, aes(x = AssoSize, y = degree))+
  geom_point()


ggplot(Fgraph2, aes(x = AssoSize, y = wdegree))+
  geom_point()




## relative weighted degree / size Asso



Fgraph2 <- fortify.tbl_graph(Fgraph2)

ggplot(Fgraph2, aes(x = AssoSize, y = relative_wdegree))+
  geom_point()
